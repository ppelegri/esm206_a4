...
## Juvenile snowshoe hare trap counts **Insert takeaways**

Count the total number of juvenile hare trappings during each year of the study and create a finalized data visualization of the counts by year.
...
hares_counts_stats <- hares_trap_counts %>% # start from hare counts df
  summarize(min_count = min(count), # calculate minimum, maximum, and median count
            max_count = max(count),
...
ggplot(data = hares, 
  ...
  scale_color_manual(values = c("chocolate2","darkolivegreen4","deepskyblue3")) +
  geom_boxplot(fill = NA, width = 0.2, outlier.color = NA) + # apply box plots, make them transparent, don't highlight outliers
  stat_summary(fun=mean, # put the mean on the graph as a round point
               geom="point",
               shape=20,
               size=3,
               color="gray14",
...
## Male and female hare weight comparison **Update explanatory text**

**Choose one: either this table or the one below**
...
# Split the above data frame into three data frames, broken up by site
hares_stats_bonbs <- hares_wt_sex_site %>% 
  filter(grid_full == "Bonanza Black Spruce")

hares_stats_bonmat <- hares_wt_sex_site %>% 
  filter(grid_full == "Bonanza Mature")

hares_stats_bonrip <- hares_wt_sex_site %>% 
  filter(grid_full == "Bonanza Riparian")


# Create a series of tables that display the summary statistics calculated in the data frames above
kable(hares_stats_bonbs,
...
**Choose one: either this table or the one above**
Create a table of summary statistics on juvenile snowshoe hare weight by sex
```{r}
...
### **Confirm that my comments are all correct**
```{r, include = FALSE}
...
### **Review results and make sure that interpretations are correct. Reference diagnostic plots.**
...
ggplot(data = hares, aes(x = hindft, y = weight)) +
  ...
  labs(x = "Hind foot length (mm)",
       y = "Weight (g)")
```
**Double check P-Value...and understand output from glance**
**Figure 3.** Relationship between hind foot length (mm) and weight (g) for juvenile snowshow hares. Points indicate individual hare measurements. Linear model summary: $\beta$~1~ = `r round(hares_lm_tidy$estimate[2], 2)` g mm^-1^, p < 0.001, R^2^ = `r round(hares_lm_glance$r.squared,2)`, Pearson's *r* = `r round(hares_cor_tidy$estimate,2)`). Data: Kielland et al. (2017).
